        ...
        <div style={{padding: "0 5%"}}>
            {components.map(v=>{
                ...
                return <div key={Math.random()}>
                    ...
                    <h2>Default</h2>
                    <div>
                        {v.children?<Component>{v.children}</Component>:<Component/>}
                    </div>
                    {
                        v.props.map(property=>{
                            const prop = property.name
                            return <div key={Math.random()} style={{paddingLeft: "5%"}}>
                                <h2><i>{prop}:</i></h2>
                                {property.values.map(value=><div key={Math.random()} style={{paddingLeft: "5%"}}>
                                    <h6>{value.toString()||"Without value"}</h6>
                                    {v.children?<Component {...{[prop]: value}}>{v.children}</Component>:<Component {...{[prop]: value}} />}
                                </div>)}
                            </div>
                        })
                    }
